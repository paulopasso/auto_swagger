...
class ApiDocParser:
    def __init__(self, js_filepath):
        ...
        with open(self.filepath, 'r', encoding='utf-8') as f:
            self.code = f.read()
        # Load spaCy model if present, fallback to basic tokenizer
        try:
            self.nlp = spacy.load("en_core_web_sm")
            # User feedback on successful model load
            print(f"spaCy model 'en_core_web_sm' loaded successfully for {self.filename}.")
        except Exception as e:
            # User feedback on fallback
            print(f"Warning: spaCy model 'en_core_web_sm' not found or failed to load ({e}) for {self.filename}. Falling back to basic tokenization. NER/POS features will be limited.")
            self.nlp = English()
            # Attempt to add a tagger for basic POS if model failed
            if not self.nlp.has_pipe("tagger"):
                 self.nlp.add_pipe("tagger", last=True)
                 try:
                    self.nlp.initialize()
                 except Exception as init_e:
                    # Feedback if tagger init fails
                    print(f"Warning: Failed to initialize POS tagger for fallback ({init_e}). POS features may be unavailable.")
    ...
    def _find_route_definitions(self):
        ...
        for m in pattern.finditer(self.code):
            routes.append({
                ...
                'handler_start': m.start('handler'), # Position where handler text starts
                'line': self.code.count('\n', 0, m.start()) + 1 # Estimate line number
            })
    ...
    def _check_if_required(self, param_name, handler_body, source):
        ...
        has_default_assignment = self._find_default_value(param_name, handler_body, source) is not None
        ...
        return source != "path" and validation_pattern.search(handler_body) is not None and not has_default_assignment
    ...
    def _find_default_value(self, param_name, handler_body, source):
        ...
        if match:
            ...
            if default_text in ('true', 'false'): return default_text == 'true'
            if default_text.isdigit(): return int(default_text)
            # Handle quoted strings
            if (default_text.startswith("'") and default_text.endswith("'")) or \
               (default_text.startswith('"') and default_text.endswith('"')) or \
    ...
    def _generate_description(self, param_name: str, source: str) -> str:
        ...
        name_lower = param_name.lower()
        if 'id' in name_lower: return f"Identifier ({param_name})."
        if 'name' in name_lower: return f"Name ({param_name})."
        if 'email' in name_lower: return f"Email address ({param_name})."
        if 'date' in name_lower or 'time' in name_lower: return f"Date or timestamp ({param_name})."
        if 'limit' in name_lower or 'page' in name_lower or 'offset' in name_lower: return f"Pagination parameter ({param_name})."
        if 'is' in name_lower or 'has' in name_lower or name_lower.startswith('enable'): return f"Boolean flag ({param_name})."
    ...
    def _extract_responses(self, handler_body: str) -> dict:
        ...
        search_area = handler_scope_match.group('scope') if handler_scope_match else handler_body
        ...
        for match in response_pattern.finditer(search_area):
            ...
            body_obj = None
            schema = {"type": "null"} if method == 'sendstatus' else {"type": "object", "properties": {}} # Default schema
            ...
            if status_code >= 400: # Error response
                ...
                if isinstance(body_obj, dict):
                    for field in ('error', 'message', 'detail', 'issue'):
                        if field in body_obj and isinstance(body_obj[field], (str, int, bool, float)):
                        ...
                        for item in body_obj['details']:
                             if isinstance(item, dict):
                                for k, v in item.items():
                                     if isinstance(v, (str, int, bool, float)): description_parts.append(f"{k}: {v}")
                             elif isinstance(item, (str, int, bool, float)): description_parts.append(str(item))
                else:
                ...
                description = "; ".join(description_parts) or f"Error response with status {status_code}."
                responses['errors'].append({'statusCode': status_code, 'description': description, 'schema': schema})
            else: # Success response
                # Extract 'message' field for description if possible
                if isinstance(body_obj, dict) and 'message' in body_obj and isinstance(body_obj['message'], str):
                ...
                responses['success'].append({'statusCode': status_code, 'description': description, 'schema': schema})

        # Consolidate success: Keep only the last detected success response for simplicity
        if responses['success']:
            responses['success'] = responses['success'][-1]
        else:
            responses['success'] = None # Explicitly set to None if no success response found
    ...
    def _infer_schema_from_response_body(self, body_content):
        ...
        if not isinstance(body_content, str):
            ...
            if isinstance(body_content, bool): return {"type": "boolean"}
            if isinstance(body_content, (int, float)): return {"type": "number"}
            if body_content is None: return {"type": "null"}
        ...
        if body_str.startswith('{') and body_str.endswith('}'):
            ...
            prop_pattern = re.compile(r'([a-zA-Z_$][\w$]*|\'[^\']*\'|"[^"]*")\s*:\s*([^,}]+(?:\{.*?\}|\[.*?\])?)')
            for key_match, value_match in prop_pattern.findall(body_str[1:-1]):
                key = key_match.strip('\'"')
                value_str = value_match.strip()
                schema["properties"][key] = self._infer_schema_from_value_string(key, value_str)
        ...
        # Infer from string looking like an array literal: [ item1, item2 ]
            ...
            if '{' in body_str and '}' in body_str: items_schema = {"type": "object"}
            elif '"' in body_str or "'" in body_str: items_schema = {"type": "string"}
            elif re.search(r'\d', body_str): items_schema = {"type": "number"}
            return {"type": "array", "items": items_schema}

        # Infer from string looking like a string literal: "hello" or 'hello'
        elif (body_str.startswith("'") and body_str.endswith("'")) or \
             (body_str.startswith('"') and body_str.endswith('"')) or \
    ...
    def _infer_schema_from_value(self, key, value):
        ...
        prop_schema = {}
        if isinstance(value, str): prop_schema = {"type": "string"}
        elif isinstance(value, bool): prop_schema = {"type": "boolean"}
        elif isinstance(value, (int, float)): prop_schema = {"type": "number"}
        elif isinstance(value, dict):
            # Recurse for nested objects
            prop_schema = {"type": "object", "properties": {}}
            for sub_key, sub_value in value.items():
                prop_schema["properties"][sub_key] = self._infer_schema_from_value(sub_key, sub_value)
        elif isinstance(value, list):
            # Recurse for arrays, inferring item type from first element
            items_schema = {"type": "any"}
            if value: # Infer from first item if list is not empty
                items_schema = self._infer_schema_from_value("", value[0]) # No key for array item
            prop_schema = {"type": "array", "items": items_schema}
        elif value is None: prop_schema = {"type": "null"}
        else: prop_schema = {"type": "any"} # Fallback for unexpected types
    ...
    def _infer_schema_from_value_string(self, key, value_str):
        ...
        if value_str.startswith("'") or value_str.startswith('"') or value_str.startswith("`"): prop_schema = {"type": "string"}
        elif value_str.isdigit() or re.match(r'parseInt\s*\(', value_str): prop_schema = {"type": "number"}
        elif value_str in ['true', 'false']: prop_schema = {"type": "boolean"}
        elif value_str == 'null' or value_str == 'undefined': prop_schema = {"type": "null"}
        elif value_str.startswith('{'): prop_schema = {"type": "object"} # Not recursing in string mode
        elif value_str.startswith('['):
            # Basic item type inference for arrays in string mode
            items_type = "any"
            if '{' in value_str: items_type = "object"
            elif "'" in value_str or '"' in value_str: items_type = "string"
            elif re.search(r'\d', value_str): items_type = "number"
            ...
            var_name = re.match(r'req\.(?:params|body|query)\.(\w+)', value_str).group(1)
            inferred_type = self._infer_parameter_type(var_name).split(" | ")[0] # Take first guess
            prop_schema = {"type": inferred_type} if inferred_type != "any" else {"type": "any"}
    ...
    def _infer_validation_constraints(self, handler_body):
        ...
        validation_libs = [
            (r'body\([\'"](\w+)[\'"]\)\.(is[A-Z]\w+|notEmpty|exists)', 'Validate $1 using express-validator: $2'), # express-validator body()
            (r'check\([\'"](\w+)[\'"]\)\.(is[A-Z]\w+|notEmpty|exists)', 'Validate $1 using express-validator: $2'), # express-validator check()
            (r'joi\.(string|number|boolean|object)\(\)(?:\.(?:required|min|max|length|email|pattern)\(.*\))+', 'Validate using Joi: $1'), # Basic Joi chain
            (r'yup\.(string|number|boolean|object)\(\)(?:\.(?:required|min|max|length|email)\(.*\))+', 'Validate using Yup: $1') # Basic Yup chain
        ]
        for pattern, template in validation_libs:
            for match in re.finditer(pattern, handler_body, re.IGNORECASE | re.DOTALL):
                ...
                constraint = template.replace('$1', field_or_type).replace('$2', method).strip()
    ...
    def _infer_resource_from_context(self, function_name, route_path):
        ...
        resource_keywords = ['user', 'product', 'order', 'item', 'account', 'post', 'comment', 'category', 'session', 'profile', 'cart', 'payment', 'address', 'book', 'article', 'event']
        ...
        if path_parts:
            ...
            singular_resource = potential_resource[:-1] if potential_resource.endswith('s') else potential_resource
            ...
            if self.nlp:
                ...
                if nouns: return nouns[0] # Fallback to first noun lemma
        ...
        if self.nlp and function_name != "anonymous":
            ...
            action_verbs = {'get', 'list', 'find', 'create', 'add', 'update', 'edit', 'delete', 'remove', 'handle', 'process', 'set', 'register'}
            ...
            if meaningful_nouns:
                return meaningful_nouns[0].capitalize() # Return first meaningful noun found
    ...
    def _infer_purpose_from_context(self, function_name, method, resource):
        ...
        if self.nlp and function_name != "anonymous" and (self.nlp.has_pipe("tagger") or "tagger" in self.nlp.pipe_names):
            doc = self.nlp(function_name)
            # Extract lemmas of Verbs, Nouns, Proper Nouns
            func_name_words = [token.lemma_.lower() for token in doc if token.pos_ in ["VERB", "NOUN", "PROPN"]]
            ...
            func_name_words = re.findall(r'[A-Z]?[a-z]+|[A-Z]+(?=[A-Z]|$)', function_name)
            func_name_words = [w.lower() for w in func_name_words]

        # Map HTTP method to a default action verb
        action_map = {"GET": "Retrieve", "POST": "Create", "PUT": "Update", "PATCH": "Partially update", "DELETE": "Delete"}
        action = action_map.get(method, "Handle") # Default to "Handle"

        # Refine action based on specific verbs found in the function name
        verb_map = {
            "get": "Retrieve", "list": "List", "find": "Find", "search": "Search", "fetch": "Fetch",
            "create": "Create", "add": "Add", "new": "Register", "post": "Post", "register": "Register",
            "update": "Update", "edit": "Edit", "modify": "Modify", "set": "Set", "patch": "Patch",
            "delete": "Delete", "remove": "Remove", "cancel": "Cancel", "process": "Process", "handle": "Handle",
            "login": "Log in", "logout": "Log out", "authenticate": "Authenticate", "authorize": "Authorize"
        }
        identified_verb = ""
        ...
        if method == "GET" and (action == "List" or "all" in func_name_words or "search" in func_name_words):
            is_plural = True
        ...
        if is_plural:
            ...
            resource_display += "s" if not resource_display.endswith("s") else ""
             ...
             if resource_display.endswith('s') and len(resource_display) > 1: # Avoid making 's' -> ''
        ...
        extra_context = " ".join(w for w in func_name_words if w not in verb_map and w != resource.lower() and w != action.lower())
    ...
    def _extract_function_name(self, handler_text):
        ...
        if re.match(r'^[a-zA-Z_$][\w$.]*$', handler_text): # Allows dot notation like controller.method
            return handler_text
        ...
        for route in self._find_route_definitions():
            ...
            for source, source_params in params.items():
                for name, info in source_params.items():
                     if info.get("required", False) and source != "path": # Path params handled via structure
                         required_constraints.append(f"Parameter '{name}' in {source} is required.")
